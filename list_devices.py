...
def get_device_items(arg, devices):
    ...
    for device in devices:
        ...
        it = Item(title=title, autocomplete=name,
                  valid=valid, arg=name, subtitle=subtitle)
...
def list_devices(args):
    ...
    for item in items:
        ...
        if arg == '' or arg.lower() in name.lower():
            it = wf.add_item(title=item.title, uid=item.title, autocomplete=('', item.autocomplete)[
                             item.valid], valid=item.valid, arg=item.arg, subtitle=item.subtitle)
            it.setvar('status', item.get("status"))
            it.setvar('full_info', item.subtitle)
            if item.valid:
                it.setvar('device_api', item.get('device_api'))
                it.setvar('ro.product.manufacturer',
                          item.get('ro.product.manufacturer'))
            ...
            if item.subtitle and not re.match(regexIp + ":5555", name) and not name.startswith("emulator-"):
                ...
                cmd_ip = adb_path + ' -s ' + name + \
                    " shell ip -f inet addr show wlan0 | grep inet | tr -s ' ' |  awk '{print $2}'"
            ...
            if lastFuncs and len(lastFuncs) > 0:
                ...
                mod = it.add_modifier(
                    "fn", subtitle="show command history", arg="cmd_history")
                mod.setvar("function", "cmd_history")
                if len(lastFuncs) > 1:
                    second_last_func = lastFuncs[len(lastFuncs) - 2]
                    mod = it.add_modifier(
                        "shift", subtitle="run 2nd last command {}".format(second_last_func))
    ...
    if arg and ("connect ".startswith(arg.lower()) or re.match(regexConnect, arg)):
        ...
        if localIp:
            ...
            valid = True if re.match(
                "^" + regexIp + "(:|:5|:55|:555|:5555)?$", targetIp) else False

            if valid:
                subtitle = "adb connect " + targetIp if targetIp else ''
                it = wf.add_item(title="Connect over WiFi",
                                 valid=valid, arg="adb_connect", subtitle=subtitle)
                m = it.add_modifier(
                    'cmd', subtitle="Remove all connection histories", arg='adb_connect_remove')
                m.setvar('extra', "all")
                ...
                for historyWifiDevice in historyWifiDevices:
                    if not historyWifiDevice.title in currentDevices:
                        ...
                        if hasattr(historyWifiDevice, 'mask') and historyWifiDevice.mask:
                            same_network = ipaddress.ip_network(u'%s/%d' % (localIp, rawMask), False) == ipaddress.ip_network(
                                u'%s/%s' % (deviceIp, historyWifiDevice.mask), False)
                        else:
                            same_network = ipaddress.ip_network(
                                u'%s/%d' % (localIp, rawMask), False) == ipaddress.ip_network(u'%s/%d' % (deviceIp, rawMask), False)
                        ...
                        if historyWifiDevice.subtitle:
                            title = "Connect " + \
                                historyWifiDevice.subtitle.split(
                                    '- ', 1)[1].split(', ', 1)[0] + " over WiFi"

                        it = wf.add_item(title=title, valid=True, arg="adb_connect", autocomplete="connect " +
                                         historyWifiDevice.title, subtitle=historyWifiDevice.title, uid=(historyWifiDevice.title, "")[valid])
                        it.setvar("ip", historyWifiDevice.title)
                        it.add_modifier('cmd', 'Remove connection history with {0}'.format(
                            historyWifiDevice.title), arg='adb_connect_remove')
            ...
            if not valid and counter == 0:
                if (not targetIp or re.match(regexIpInput, targetIp)):
                    subtitle = "adb connect " + targetIp if targetIp else ''
                    if not targetIp:
                        it = wf.add_item(title="Connect over WiFi", valid=False,
                                         arg="adb_connect", autocomplete="connect ", subtitle=subtitle)
                    else:
                        it = wf.add_item(
                            title="Connect over WiFi", valid=False, arg="adb_connect", subtitle=subtitle)
    ...
    if arg and ("disconnect ".startswith(arg.lower()) or re.match("^disconnect .*", arg)):
        ...
        if wifiDevices:
            for wifiDevice in wifiDevices:
                it = wf.add_item(title="Disconnect from WiFi", uid=wifiDevice.title, valid=True,
                                 arg="adb_disconnect", autocomplete="disconnect ", subtitle=wifiDevice.title)
                ip = wifiDevice.title
                if "[OFFLINE]" in ip:
                    ip = ip.split(" ")[0]
            ...
            it = wf.add_item(title="Disconnect from WiFi", uid="adb_disconnect", valid=True,
                             arg="adb_disconnect", autocomplete="disconnect ", subtitle="adb disconnect " + targetIp)
    ...
    if arg and ("restart".startswith(arg.lower())
        ...
        wf.add_item(title="Restart adb", valid=True,
                    arg="restart_adb", uid="restart_adb")
